...
public abstract class AbstractIdNameActiveModelController<T extends IdNameActiveModel> {
    ...
    private final ProcessIdNameActiveModelService<T> processCustomerService;

    AbstractIdNameActiveModelController(ProcessIdNameActiveModelService<T> processCustomerService) {
    ...
    private static Boolean getActiveRequired(@RequestBody ObjectNode objectNode) {
        ...
        if (null == aBoolean) {
            throw new ResponseStatusException(HttpStatus.UNPROCESSABLE_ENTITY, AbstractIdNameActiveModelController.ACTIVE_REQUIRED_MESSAGE);
        }
        return aBoolean;
    }
